...
## Required flow
1. Derive a short case title from the active issue.
2. Create or update `.rm-logs/YYYYMMDD - titulo-curto.md` with `scripts/upsert-rm-log.(ps1|sh)`.
3. At the end of analysis, append a summary with status `em_analise` or `solucao_proposta`.
4. Only when the RM explicitly confirms resolution, append another entry with status `solucionado_confirmado`.
5. Use `assets/log-entry.template.md` and `references/logging-rules.md`.
